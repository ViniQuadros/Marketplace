...
export default function ProductCarousel({ products, loading, error }) {
  ...
  if (loading) return <p className="carousel-message">Carregando produtos...</p>;
  if (error) return <p className="carousel-error">{error}</p>;
  if (!products || products.length === 0) return <p className="carousel-message">Nenhum produto encontrado.</p>;
